...
def test_create_swimming_workout_returns_id(garmin_mock):
    ...
    garmin_mock.upload_swimming_workout.assert_called_once()
...
def test_invalid_json_returns_error(garmin_mock):
    result = json.loads(create_swimming_workout("not json"))
    assert "error" in result


def test_step_structure(garmin_mock):
    create_swimming_workout(_BASIC_WORKOUT)
    workout = garmin_mock.upload_swimming_workout.call_args[0][0]
...
def test_repeat_group_inner_steps(garmin_mock):
    create_swimming_workout(_BASIC_WORKOUT)
    workout      = garmin_mock.upload_swimming_workout.call_args[0][0]
    repeat_group = workout.workoutSegments[0].workoutSteps[2]
...
def test_pool_length_is_set(garmin_mock):
    create_swimming_workout(_BASIC_WORKOUT)
    workout = garmin_mock.upload_swimming_workout.call_args[0][0]
    assert workout.model_extra["poolLength"] == 25.0
    assert workout.model_extra["poolLengthUnit"]["unitKey"] == "meter"


def test_sport_type_is_swimming(garmin_mock):
    create_swimming_workout(_BASIC_WORKOUT)
    workout = garmin_mock.upload_swimming_workout.call_args[0][0]
    assert workout.sportType["sportTypeId"] == 4
